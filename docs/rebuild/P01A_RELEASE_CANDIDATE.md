...
- **Triggers**: `pull_request` targeting `main`, `push` to `main`, and manual `workflow_dispatch` — exactly as specified, nothing broader (no `pull_request_target`, no wildcard branches). **Correction (P01A.3, see §11): manual `workflow_dispatch` is not actually usable until this workflow file exists on the default branch** — GitHub returns `workflow ci.yml not found on the default branch` for any workflow that has never been merged to `main`, regardless of the `workflow_dispatch:` trigger being declared. This is a GitHub platform constraint, not a configuration mistake in this file.
...
## 11. P01A.3 correction — CI activation before merge

**On manual dispatch:** §3 originally listed `workflow_dispatch` as usable immediately. In practice, `gh workflow run ci.yml` fails with `HTTP 404: workflow ci.yml not found on the default branch` for any workflow file that has never existed on `main` — GitHub's workflow registry (the same one backing `workflow_dispatch` and the `actions/workflows` list endpoint) is populated from the default branch, independent of the `workflow_dispatch:` trigger being declared in the file itself. This is expected GitHub behavior, not a defect in this workflow.

**Safe pre-merge validation:** a `push` event to the exact stabilization branch does not depend on that registry — `push` triggers evaluate the workflow file as it exists at the pushed commit directly. Temporarily adding `fix/p01a-stabilization-clean` to the `push.branches` list (see the inline comment in `ci.yml`) makes it possible to obtain a real GitHub Actions run against this branch's own HEAD commit before the workflow ever touches `main`, with no merge and no deployment involved. This branch name should be removed once the workflow lands on `main` and a normal run has been observed there.

**Permanent-domain and disputed-content status are not blockers for *this* stabilization release.** They were listed as open items in §10 in the sense that they block the *eventual, permanent* portfolio launch — but they do not block this branch's own release-candidate status, because both are already handled safely within this branch's own scope:

- The canonical-domain requirement for this release is satisfied by the **approved temporary canonical**, `https://shahriyarkhan.vercel.app` — not by the still-open permanent-domain decision. Every public metadata reference already uses it (§ `P01A_STABILIZATION_REPORT.md` §6), and CI/local checks already assert its presence and the absence of `shahriyarkhan.dev`.
- CognoRise and InsightBoard are **deliberately hidden**, not pending removal — their non-public status is itself the correct, already-implemented state for this release (§ `P01A_STABILIZATION_REPORT.md` §7). Nothing about resolving their underlying authenticity questions is required to release *this* stabilization work.

These two items remain real blockers only for the **final portfolio content/rebuild decisions** (a permanent domain, and whether CognoRise/InsightBoard are ever un-hidden) — not for shipping this stabilization branch's bug fixes and test suite.
