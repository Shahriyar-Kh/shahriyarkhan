...
**Runtime: Node 22.22.2 exactly**, pinned in `web/.nvmrc`, `web/package.json`'s `engines.node` (`^22.22.2`), and `.github/workflows/ci.yml`'s `web` job (`node-version-file: web/.nvmrc`). This is a hard requirement discovered the hard way: this app's dependency tree includes `jsdom@30` (`^22.22.2 || ^24.15.0 || >=26.0.0`) and `undici@8.10` (`>=22.19.0`) — the CI job originally shipped pinned to Node 20 (matching the *scaffold* commit's now-too-permissive `engines.node: ">=20.9.0"`), which passed `npm ci` (npm only warns on `EBADENGINE`, it doesn't fail) but then made every Vitest worker crash before running a single test, with `TypeError: webidl.util.markAsUncloneable is not a function` thrown from inside `undici`'s `CacheStorage` constructor. 22.22.2 — the oldest patch in the Node 22 LTS line that satisfies `jsdom`'s range — was chosen over the 24.x line specifically because it matches `../frontend/package.json`'s own existing `"engines.node": ">=22.12.0"` convention; nothing in this repository favors 24.x over 22.x.
